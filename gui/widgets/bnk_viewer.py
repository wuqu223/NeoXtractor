    ...
    def __init__(self, data: bytes):
        ...
        try:
            ...
            if f.read('bytes:4') == b"DIDX":
                ...
                files = didx_len // 12
                print(f"There are {files} WAV files")
                ...
                for id, offset, len in extract_list:
                    ...
                    #open(f"{hex(wem_list[cont][0])}.wav", "wb").write(wem_list[cont][1])
            else:
                self.clear_files()
...
class BnkViewer(QtWidgets.QWidget):
    ...
    def __init__(self):
        ...
        self.v_layout.addWidget(self.msg_box)
    ...
    def save_wem_file(self, item):
        if self.container:
            ...
            if content is not None:
                ...
                self.msg_box.setText("File '{item.text()}'.wem saved successfully.")
            else:
